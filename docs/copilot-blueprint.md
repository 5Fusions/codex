...
2. Copy `docs/primator-config.example.toml` to `~/.codex/config.toml` and tweak the provider/preamble if you like.
...
2. Paste the minimal Primator profile into `~/.codex/config.toml` (see "Quickstart" below).
3. Run `codex --profile primator` and approve each command prompt (`approval_policy = "untrusted"`).
4. Swap `model_provider` to `ollama` for offline use or keep `openai`/cloud providers for online.
5. (Optional) Pipe microphone transcripts into the running CLI and send replies through TTS for voice-only sessions.
...
2. **Create a minimal Primator profile**
   ```bash
   mkdir -p ~/.codex
   cp docs/primator-config.example.toml ~/.codex/config.toml
   ```

   - The example file already includes OpenAI plus optional Ollama/Gemini/DeepSeek providers; flip `model_provider` to the one you actually use.
...
## Quickstart (Android via Termux)

If you want Primator on a phone or tablet without a desktop, these Termux steps mirror the laptop flow:
...
1. **Install Codex CLI**
   ...
   # Fetch the latest ARM64 binary (adjust the URL for a newer release)
   curl -L -o ~/codex "https://github.com/openai/codex/releases/latest/download/codex-aarch64-unknown-linux-musl"
   chmod +x ~/codex
   mkdir -p ~/bin && mv ~/codex ~/bin/

   # Make sure ~/bin is on PATH for future shells
   echo 'export PATH="$HOME/bin:$PATH"' >> ~/.bashrc
   source ~/.bashrc
   ```

2. **Create a minimal Primator profile** (same as desktop)
   ```bash
   mkdir -p ~/.codex
   cp ~/codex/docs/primator-config.example.toml ~/.codex/config.toml
   ```
...
3. **Run Primator**
   ...
   codex --profile primator
   ```
   - Type clear task directives ("initialize a Python project") and approve shell commands as prompted.

4. **Notes for mobile use**
   - Keep `approval_policy = "untrusted"` so accidental taps never trigger commands.
   - If you need offline inference on-device, pair Termux with a LAN-accessible model server; most LLM servers are heavy for phonelike hardware.
   - Voice I/O still works by piping microphone transcripts into the running `codex` process and reading responses with TTS.
